...
namespace ProgrammingAssignment6
{
    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        Vector2 hitButtonLocation = new Vector2(HORIZONTAL_MENU_BUTTON_OFFSET, TOP_MENU_BUTTON_OFFSET);
        Vector2 standButtonLocation = new Vector2(HORIZONTAL_MENU_BUTTON_OFFSET, TOP_MENU_BUTTON_OFFSET + VERTICAL_MENU_BUTTON_SPACING);
        Vector2 quitButtonLocation = new Vector2(HORIZONTAL_MENU_BUTTON_OFFSET, QUIT_MENU_BUTTON_OFFSET);
        ...
        protected override void LoadContent()
        {
            ...
            MenuButton hitButton = new MenuButton(hitButtonSprite, hitButtonLocation, GameState.PlayerHitting);
            menuButtons.Add(hitButton);

            // create stand button and add to list
            standButtonSprite = Content.Load<Texture2D>("standbutton");
            MenuButton standButton = new MenuButton(standButtonSprite, standButtonLocation, GameState.WaitingForDealer);
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            switch (currentState)
            {
                case GameState.PlayerHitting:
                    ...
                    Card card = deck.TakeTopCard();
                    card.X = HORIZONTAL_CARD_OFFSET;
                    card.Y = TOP_CARD_OFFSET + (playerHand.Count()) * VERTICAL_CARD_SPACING;
                    card.FlipOver();
                    playerHand.Add(card);
                ...
                case GameState.DealerHitting:
                    ...
                    card = deck.TakeTopCard();
                    card.X = WINDOW_WIDTH - HORIZONTAL_CARD_OFFSET;
                    card.Y = TOP_CARD_OFFSET + (dealerHand.Count()) * VERTICAL_CARD_SPACING;
                    card.FlipOver();
                    dealerHand.Add(card);
                    currentState = GameState.CheckingHandOver;
                    break;
                case GameState.CheckingHandOver:
                    // if either busted or both standing
                    if (GetBlackjackScore(playerHand) > MAX_HAND_VALUE || GetBlackjackScore(dealerHand) > MAX_HAND_VALUE ||
                        !playerHit && !dealerHit)
                    {
                        if (GetBlackjackScore(playerHand) > MAX_HAND_VALUE && GetBlackjackScore(dealerHand) > MAX_HAND_VALUE)
                        {
                            messages[1].Text = "Both busted";
                        }
                        else if (GetBlackjackScore(playerHand) > MAX_HAND_VALUE)
                        {
                            messages[1].Text = "Player busted";
                        }
                        else if (GetBlackjackScore(dealerHand) > MAX_HAND_VALUE)
                        {
                            messages[1].Text = "Dealer busted";
                        }
                        else if (GetBlackjackScore(playerHand) > GetBlackjackScore(dealerHand))
                        {
                            messages[1].Text = "Player wins";
                        }
                        else if (GetBlackjackScore(playerHand) < GetBlackjackScore(dealerHand))
                        {
                            messages[1].Text = "Dealer wins";
                        }
                        else 
                        {
                            messages[1].Text = "It's a tie!";
                        }
                        currentState = GameState.DisplayingHandResults;
                        dealerHand[0].FlipOver();
                        menuButtons.Clear();
                        quitButton = new MenuButton(quitButtonSprite, quitButtonLocation, GameState.Exiting);
